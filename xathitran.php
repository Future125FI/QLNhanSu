...
if (isset($_POST['submit'])) {
    ...
    if (empty($tendonvi) || empty($madonvi)) {
        ...
        $query = "INSERT INTO xathitran (MaDV, Tendonvi) VALUES ('$madonvi', '$tendonvi')";
        $result = mysql_query($query);
        if ($result) {
            echo "<p id='add-success-message' style='color: green;'>Thêm xã mới thành công.</p>";
...
if (isset($_GET['edit'])) {
    ...
    if (isset($_POST['update'])) {
        ...
        if ($tendonvi_new != '') {
            $query_update = "UPDATE xathitran SET Tendonvi='$tendonvi_new' WHERE MaDV='$edit_id'";
            $result_update = mysql_query($query_update);
            if ($result_update) {
                echo "<p id='edit-success-message' style='color: green;'>Sửa thông tin xã thành công.</p>";
            } else {
                echo "<p style='color: red;'>Lỗi: " . mysql_error() . "</p>";
            }
        } else {
...
if (isset($_GET['delete'])) {
    ...
    $sql = "DELETE FROM xathitran WHERE MaDV = '$delete_id'";
    $result = mysql_query($sql);
    if ($result) {
       echo "<p id='delete-success-message' style='color: green;'>Xóa xã thành công.</p>";
    } else {
        echo "<p style='color: red;'>Lỗi: " . mysql_error() . "</p>";
    }
}

// Truy vấn dữ liệu từ bảng 'xathitran'
$result = mysql_query("SELECT * FROM xathitran");
...
if (mysql_num_rows($result) > 0) {
    ...
    while ($row = mysql_fetch_assoc($result)) {
        ...
        $redirect_url = "";
        switch ($row["Tendonvi"]) {
    case "Thị trấn Đại Ngãi":
        $redirect_url = "tv_danhmucthitrandaingai.php?MaDV=";
        break;
    case "Xã Long Phú":
        $redirect_url = "tv_danhmucxalongphu.php?MaDV=";
        break;
    case "Xã Tân Hưng":
        $redirect_url = "tv_danhmucxatanhung.php?MaDV=";
        break;
    case "Xã Tân Thạnh":
        $redirect_url = "tv_danhmucxatanthanh.php?MaDV=";
        break;
    case "Xã Long Đức":
        $redirect_url = "tv_danhmucxalongduc.php?MaDV=";
        break;
    case "Xã Châu Khánh":
        $redirect_url = "tv_danhmucxachaukhanh.php?MaDV=";
        break;
    case "Xã Phú Hữu":
        $redirect_url = "tv_danhmucxaphuhuu.php?MaDV=";
        break;
    case "Xã Song Phụng":
        $redirect_url = "tv_danhmucxasongphung.php?MaDV=";
        break;
    case "Xã Hậu Thạnh":
        $redirect_url = "tv_danhmucxahauthanh.php?MaDV=";
        break;
    case "Xã Trường Khánh":
        $redirect_url = "tv_danhmucxatruongkhanh.php?MaDV=";
        break;
    // Thêm các trường hợp khác nếu cần
    default:
        // Mặc định nếu không khớp với các trường hợp trên
        $redirect_url = "tv_danhmucthitranlongphu.php?MaDV=";
}


        // Hiển thị hàng của bảng với URL chuyển hướng đã xác định
        echo "<tr><td>" . $row["MaDV"] . "</td><td><a class='ten-don-vi' href='" . $redirect_url . $row["MaDV"] . "'>" . $row["Tendonvi"] . "</a></td><td><a href='xathitran.php?edit=" . $row["MaDV"] . "'>Sửa</a> | <a href='xathitran.php?delete=" . $row["MaDV"] . "'>Xóa</a></td></tr>";
    }
    echo "</table>";
} else {
...
// Form thêm cấp xã mới
echo "<h2>Thêm cấp xã mới:</h2>";
echo '<form method="post" action="">';
